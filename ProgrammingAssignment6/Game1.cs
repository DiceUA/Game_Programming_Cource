...
namespace ProgrammingAssignment6
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            deck = new Deck(Content, WindowWidth / 2, 100);
            deck.Shuffle();
            ...
            menuButtons.Add(new MenuButton(hitButtonSprite, new Vector2(WindowWidth / 2, TopCardOffset), GameState.PlayerHitting));

            // create stand button and add to list
            standButtonSprite = Content.Load<Texture2D>(@"graphics\standbutton");
            menuButtons.Add(new MenuButton(standButtonSprite, new Vector2(WindowWidth / 2, TopCardOffset + VerticalCardSpacing), GameState.WaitingForDealer));
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {                
                case GameState.CheckingHandOver:
                    ...
                    if(GetBlockjuckScore(playerHand) > 21 || GetBlockjuckScore(dealerHand) > 21)
                    {
                        if(GetBlockjuckScore(playerHand) > 21 && GetBlockjuckScore(dealerHand) > 21)
                        {
                            // Tie
                        ...
                        else if (GetBlockjuckScore(playerHand) > 21 && GetBlockjuckScore(dealerHand) <= 21)
                        {
                            // Player LOSS
                        ...
                        menuButtons.Clear();
                        menuButtons.Add(new MenuButton(quitButtonSprite, new Vector2(WindowWidth / 2, WindowHeight - 100), GameState.Exiting));
                    ...
                    else
                    {
                        ...
                        // Only player stand?
                        // Make dealer draw cards so player dont need to press Stand button after each dealer card
                        else if(!playerHit)
                        {
                ...
                case GameState.DealerHitting:
                    ...
                    card = deck.TakeTopCard();
                    card.X = WindowWidth - HorizontalCardOffset;
                    card.Y = TopCardOffset + VerticalCardSpacing * dealerHand.Count;
                    card.FlipOver();
                    dealerHand.Add(card);
                    ChangeState(GameState.WaitingForDealer);
                    break;
                case GameState.DisplayingHandResults:
                    // Nothig should be here
                    break;
                ...
                case GameState.PlayerHitting:
                    ...
                    card = deck.TakeTopCard();
                    card.X = HorizontalCardOffset;
                    card.Y = TopCardOffset + VerticalCardSpacing * playerHand.Count;
                    card.FlipOver();
                    playerHand.Add(card);
                    messages.Clear();                    
